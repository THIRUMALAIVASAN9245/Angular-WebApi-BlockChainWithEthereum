...
{
     //Set of States
    enum StateType { PolicyRequest, PolicyRespondInValid, PolicyRespondValid, SubmitBillDetailsRequest, SubmitBillDetailsResponse, RejectClaimRequest, ApprovedClaimRequest, HealthDBApprovalRequest }
	...
	// call this function to return policy status 
    function SubmitBillDetailsRequest(string memory treatmentType, string memory billNumber,string memory billAmount, string memory description) public
    {
        Insurer = msg.sender;

        // call ContractUpdated() to record this action
        ResponseMessage = treatmentType;
		ResponseMessage = billNumber;
		ResponseMessage = billAmount;
